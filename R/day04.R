...
parse_card <- function(x) {
  lapply(x, function(card) {
    aux <- strsplit(card, ":", fixed = 2)[[1]]
    card_id <- as.numeric(gsub("\\D", "", aux[1]))
    numbers <- strsplit(aux[2], "|", fixed = 2)[[1]]
    numbers_clean <- sapply(numbers, gsub, pattern = "\\s+", replacement = " ")
    winner <- strsplit(trimws(numbers_clean[1]), " ")[[1]]
    obtained <- strsplit(trimws(numbers_clean[2]), " ")[[1]]
    list(id = card_id, winner = winner, obtained = obtained)
  })
}
