...
f06a <- function(x) {
  records <- data.frame(
    time = parse_numbers(x, "Time:"),
    distance = parse_numbers(x, "Distance:")
  )
  winning_options_c <- sapply(seq_len(nrow(records)),
         \(i) winning_options(records$time[i], records$distance[i]))
  prod(winning_options_c, na.rm = TRUE)
}
...
f06b <- function(x) {
  ...
  time <- as.numeric(paste0(records$time, collapse = ""))
  distance <- as.numeric(paste0(records$distance, collapse = ""))
